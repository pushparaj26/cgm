...
def perform_analysis(glucose):
   ...
   # Glucose Matrices calculation
    ...
    if genred=='Official ADA recommendation for a person with diabetes':
       ...
       for i, row in glucose[glucose["Event type / Notif type"] == 'MEAL'].iterrows():
             fig_GR.add_shape( x0=row['Actual time'], y0=max(glucose['Glucose reading']), x1=row['Actual time'], y1=max(glucose['Glucose reading']),
                 line=dict(color="green", width=2))
       st.plotly_chart(fig_GR, use_container_width=True)
       
       fig_FR = px.line(food_d, x="Actual time", y=["Event type / Notif type", 'Event name'])
...
which is a medical emergency and needs to be treated immediately''')
    ...
    pio.write_image(figz , "graph_pie.png", format='png', width=600, height=800, scale=4)
    pio.write_image(fig_group , "graph_grop.png", format='png', width=800, height=600, scale=3)
    pio.write_image(fipB , "graph_B.png", format='png', width=800, height=600, scale=3)
    ...
    pdf1 = pdfkit.from_string(html, False, options={"enable-local-file-access": ""}, css='About.css')
 
 
    st.success("🎉 Your report generated!")
        # st.write(html, unsafe_allow_html=True)
        # st.write("")
    st.download_button("⬇️ Download PDF", data=pdf1,
            file_name="cgm.pdf",
...
# Create a function to create the PDF

    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("Arial", size=12)
    pdf.cell(200, 10, txt="Hello, PDF!", ln=1, align="C")
    pdf.image("graph_pie.png", x=10, y=20, w=80, h=80)
    pdf.add_page()
    pdf.image("graph_grop.png", x=10, y=100, w=210, h=100)
    pdf.add_page()
    pdf.image("graph_B.png", x=10, y=100, w=210, h=100)
    pdf.output("document.pdf")

# Create a button to trigger the PDF creation
    ...
    with open("document.pdf", "rb") as pdf:
          ...
          st.download_button("Here is your document:", pdf, file_name="repm.pdf", key='download-button'
            )
   
food_option = st.sidebar.checkbox('Is food data available')     

def open_dash(df):
    cgmdf = preprocess_data(df)
    global food_option
    if food_option: cgmdf = food_preprocess_data(df)
